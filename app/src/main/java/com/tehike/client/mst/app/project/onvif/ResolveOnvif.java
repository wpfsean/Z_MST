...
public class ResolveOnvif extends Thread {
    ...
    //构造方法
    public ResolveOnvif(Device device, GetRtspCallback listern) {
        this.device = device;
    ...
    public void run() {
        //获取ip
        String ip = device.getVideoBen().getIp();
        if (TextUtils.isEmpty(ip)) {
            // Log.e("TAG", "Onvif对象的ip为空");
            listern.getDeviceInfoResult("", false, device);
            return;
        }
        //ping一下ip
        boolean isPingConnect = HttpUtil.isIpReachable(ip);
        if (!isPingConnect) {
        ...
        try {
            ...
            String media_Service = device.getMediaUrl();
            ...
            device.setMediaUrl(media_Service);
            //获取profile（带鉴权）
            ...
            //解析所有的token
            device.addProfiles(getMediaProfiles(ProfileResult));
            ...
            String token = device.getProfiles().get(aisle).getToken();
            //给对象设置正常的token值
            device.getVideoBen().setToken(token);

            //拼加获取rtsp的参数
            String getRtspParamater = String.format(GET_URI, device.getVideoBen().getUsername(),
            ...
            if (!TextUtils.isEmpty(rtsp)) {
                if (!rtsp.contains("@")) {
                    ...
                    device.getVideoBen().setRtsp(newUrl);
                }
            }
            //拼加截图参数
            String getShotParamater = String.format(GET_SHOT, device.getVideoBen().getUsername(),
                    mAuthPwd, mNonce, mCreated, token);
            //请求截图的url
            ...
            device.setShotPicUrl(shotPic_url);

            //最后回调
            listern.getDeviceInfoResult(rtsp, true, device);
        } catch (Exception e) {
            Log.e("TAG", "Onvif解析异常-->>" + e.getMessage());
            Logutils.d("Onvif-->>" + device.getVideoBen().getName() + "\t" + device.getVideoBen().getIp() + "\t" + device.getVideoBen().getChannel());
        }
    }
